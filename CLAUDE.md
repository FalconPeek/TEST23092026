...
- Rule A: (1) **Score first** — at least one reporter from each side; all score reports must agree, otherwise `disputed`. (2) Per player per stat: median of all reports about that player (the subject's own report counts once); a lone self-report is accepted. (3) Consistency: the agreed score is authoritative. Σ goals(team) + Σ own_goals(opponent) must be **≤** team score (goals may stay unattributed: a score-only match finalizes) and Σ assists(team) ≤ team score − own_goals(opponent); over-claims drop least-supported claims (fewest reporters) until consistent; if still inconsistent → `disputed`. After finalization a group admin can assign unattributed goals (and fix assists/own goals/saves) with `amend_match_stats` (never above the score; audited in `match_audit`; newly earned badges are awarded, none revoked; score/OpenSkill/MVP unchanged). (4) Report window 48 h, rating window 72 h (configurable); on expiry pg_cron marks `pending_finalize`. The finalizer only processes a `pending_finalize` match once `rating_deadline` has passed (MVP/form need the ratings); `request_finalize` (admin, early) closes both windows immediately.
