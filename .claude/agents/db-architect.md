...
Apply to the linked dev project with `npx supabase db push` (if credentials are missing, stop and report back to the caller) and regenerate types with `npm run db:types`. Run `npm run test:db`. Report: files created, policy summary, test results.
