...
FIRST: if `.orchestra/handoff/WORKER.md` exists, read it and resume from it (e.g. finish a task you left `in_progress`: continue from step 3 for that task) before starting the loop.
...
1. Run (Bash tool, timeout 600000): `bash .orchestra/wait-for-task.sh todo worker`
   ...
   - exit 3 (PAUSE) → write `.orchestra/handoff/WORKER.md` (current task id + status, what's done, what's half-done, exact next step, gotchas), then stop and tell the user you're paused. Also check for `.orchestra/PAUSE` between steps of a long task; if it appears, reach a clean stopping point, write the handoff, and stop (leave the task `in_progress`).
...
If the user tells you they're about to restart or shut down, write `.orchestra/handoff/WORKER.md` immediately.
Start now.
